...
from personal.view.view_correos_electronicos import Ui_Dialog_Correos
from personal.view.view import ICO_APLICACION

class Dialog_Correos(QtWidgets.QDialog):
    
    def __init__(self, id_ = None, persona_id = None, correo = None,
                 preferencia = None, observ = None):
        """Inicializa el diálogo de configuración de mails"""
        
        super(Dialog_Correos, self).__init__()
        self.ui = Ui_Dialog_Correos()
        ...
        if id_ is None:
            ...
            self.ui.lineEdit_correo.setText(str(self.ret['correo']))
    ...
    def OnTerminar(self, operacion):
        ...
        if operacion == "c":
            ...
            if operacion == "a":
                ...
                mail = self.ui.lineEdit_correo.text().strip()
                preferencia = 1 \
                ...
                if self.ret['id_'] is None:
                    ...
                    r = bd.alta_mail(self.ret['persona_id'], mail,
                                     preferencia, observ)
                    aux = "crear"
                    ...
                    r = bd.modificar_mail(self.ret['id_'], mail, preferencia,
                                          observ)
                    aux = "modificar"
            ...
            if seguir:
                ...
                if r[0]:
                    ...
                    if operacion == "a":
                        if preferencia == 1:
                            bd.modificar_preferencia_mail(r[1],
                                                          self.ret['persona_id'])
                    
                    self.ret['operacion'] = True
                
                else:
                    #self.mostrar_mensaje(f"Fallo al {aux} el mail", \
                    #                                 caja_texto = False)
            
                    msg = f"No se ha podido {aux} el correo electrónico"
